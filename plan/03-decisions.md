...
| 20 | Signups caching | **Redis read-through, Phase B built.** Current-week signups are served from a cached `{ weekStart, rows }` payload (`tdl:signups:current`, `:test` in `TEST_MODE`, 2-hour TTL) via `signupUtils.getCurrentWeekSignups`; the new **`/recentsignups`** read command consumes it (+ the roster cache for Data Names). `/signup` **evicts** the key after a successful upsert so the list reflects the new signup immediately. The `weekStart` stamp guards against serving a list cached just before the Tuesday rollover into the new week. Read/display only — the `/signup` upsert still reads `Registration!A:E` **fresh** (never a cached write target). `REG_TAB`/`COL` moved to `signupUtils` as the single source of truth. Redis down → live-Sheets fallback. See `07-redis-caching.md`. |
